...
//
// The bytes will transmitted be as 8 data bits, Even Parity bit, 1 stop bit
// (8E1) at the specified baud rate.
//
// Before calling uart_init, make sure to set up the GPIO pins: TX to
// AF_PP/10MHz. RX to IN FLOATING or Pull-up. and to enable the USART in the RCC
// register:	RCC->APBxENR |= RCC_APBxENR_USARTyEN;
static inline void uart_init(struct USART_Type *usart, int baud) {
  usart->CR1 = 0;
  usart->CR2 = 0;
  usart->CR3 = 0;
  usart->BRR = uart_brr(baud);
