...
struct Ringbuffer {
  uint8_t buf[1 << 9]; // size must be power of two for efficiency in taking
                       // modulo index
...
// invariant:
// as long as you only put_head() if !full(), and get_tail() if !empty(), tail
// <= head and head-tail < sizeof buf.

inline uint16_t ringbuffer_avail(const struct Ringbuffer *rb) {
  return rb->head - rb->tail;
} // 0..size characters available
...
inline int ringbuffer_full(const struct Ringbuffer *rb) {
  return sizeof rb->buf < (uint16_t)(rb->head - rb->tail);
}
inline void ringbuffer_clear(struct Ringbuffer *rb) {
  rb->head = rb->tail;
} // may race with get_tail and put_head
...
inline uint8_t ringbuffer_get_tail(struct Ringbuffer *rb) {
  return rb->buf[rb->tail++ % (sizeof rb->buf)];
}
