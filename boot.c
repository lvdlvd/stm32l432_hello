...
void Reset_Handler(void) {
	...
	// enable HSE on CK_IN (PA0) and wait for ready
...
// when this irq has a higher priority than all userspace, we may consider
// update to be atomic clockticks holds the counts at the _end_ of the next
// systick, so cyclecount can just subtract current VAL (which counts down)
